    ...
    int myRank, int numProcesses, GridFragment *frag, double omega, double epsilon)
{
    ...
    int num_color_cols = (frag->gridDimension + 1) / 2;
    int last_row = frag->lastRowIdxExcl - frag->firstRowIdxIncl + 1;
    do
    {
        maxDiff = 0.0;
        MPI_Request reqs[2][2][2]; // {send, recv} x {color=0, color=1} x {top_row, bottom_row}
        if (myRank > 0)
        {
            MPI_Irecv(frag->data[0][0], num_color_cols, MPI_DOUBLE, myRank - 1, 0, MPI_COMM_WORLD, &reqs[1][0][0]);
            MPI_Irecv(frag->data[1][0], num_color_cols, MPI_DOUBLE, myRank - 1, 1, MPI_COMM_WORLD, &reqs[1][1][0]);
        }
        if (myRank < numProcesses - 1)
        {
            MPI_Irecv(frag->data[0][last_row], num_color_cols, MPI_DOUBLE, myRank + 1, 2, MPI_COMM_WORLD, &reqs[1][0][1]);
            MPI_Irecv(frag->data[1][last_row], num_color_cols, MPI_DOUBLE, myRank + 1, 3, MPI_COMM_WORLD, &reqs[1][1][1]);
        }
        for (int color = 0; color < 2; ++color)
        {
            if (myRank > 0)
            {
                MPI_Isend(frag->data[!color][1], num_color_cols, MPI_DOUBLE, myRank - 1, 2 + !color, MPI_COMM_WORLD, &reqs[0][!color][0]);
                MPI_Request_free(&reqs[0][!color][0]);
            }
            if (myRank < numProcesses - 1)
            {
                MPI_Isend(frag->data[!color][last_row - 1], num_color_cols, MPI_DOUBLE, myRank + 1, !color, MPI_COMM_WORLD, &reqs[0][!color][1]);
                MPI_Request_free(&reqs[0][!color][1]);
            }
            // could use waitall for the case when the process is neither first or last
            if (myRank > 0)
            {
                MPI_Wait(&reqs[1][!color][0], MPI_STATUS_IGNORE);
            }
            if (myRank < numProcesses - 1)
            {
                MPI_Wait(&reqs[1][!color][1], MPI_STATUS_IGNORE);
            }
            ...
            // We can free the requests instead
            // if (myRank > 0)
            //{
            //     MPI_Wait(&reqs[0][!color][0], MPI_STATUS_IGNORE);
            // }
            // if (myRank < numProcesses - 1)
            //{
            //     MPI_Wait(&reqs[1][!color][1], MPI_STATUS_IGNORE);
