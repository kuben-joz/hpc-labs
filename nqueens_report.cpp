...
void recursive_solve_par(Board &partial_board, int col, oneapi::tbb::concurrent_queue<vector<int>>& solutions)
{
    ...
    else if (col < par_depth)
    {
        ...
        for (int tested_row = 0; tested_row < b_size; tested_row++)
        {
            ...
            {
                g.run([&partial_board, col, &solutions, tested_row]
                      { Board board_cp(partial_board.begin(), partial_board.end());
                        board_cp[col] = tested_row;
                        recursive_solve_par(board_cp, col + 1, solutions); });
            }
        }
        g.wait();
    }
    else
    {
        for (int tested_row = 0; tested_row < b_size; tested_row++)
        {
            if (check_col(partial_board, col, tested_row))
            {
                ...
                recursive_solve_par(partial_board, col + 1, solutions);
            }
        }
    }
}

void recursive_solve_seq(Board &partial_board, int col, vector<vector<int>> &solutions)
{
    // std::cout << "rec solve col " << col << std::endl;
    // pretty_print(b_partial);
    ...
    if (col == b_size)
    {
        ...
        solutions.emplace_back(partial_board.begin(), partial_board.end());
    }
    else
    {
        for (int tested_row = 0; tested_row < b_size; tested_row++)
...
int main(int argc, char **argv)
{
    ...
    oneapi::tbb::concurrent_queue<vector<int>> solutions_par;
    ...
    std::cout << "solution count: " << solutions_seq.size() << std::endl;
