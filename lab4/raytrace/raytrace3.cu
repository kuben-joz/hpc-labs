...
__global__ void kernel(Sphere *spheres, unsigned char *bitmap)
{
	...
	__shared__ Sphere shared_spheres[SPHERES];

	for(int i = (threadIdx.x * blockDim.y) + threadIdx.y ; i < SPHERES; i+=blockDim.x*blockDim.y) {
	...
	{
		float colorFalloff;
		float depth = shared_spheres[i].hit(bitmapX, bitmapY, &colorFalloff);

		if (depth > maxDepth)
		{
			red = shared_spheres[i].red * colorFalloff;
			green = shared_spheres[i].green * colorFalloff;
			blue = shared_spheres[i].blue * colorFalloff;
			maxDepth = depth;
		}
	}
