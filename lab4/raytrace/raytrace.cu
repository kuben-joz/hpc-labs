...
__global__ void kernel(unsigned char *bitmap)
{
	...
	{
		float colorFalloff;
		float depth = devSpheres[i].hit(bitmapX, bitmapY, &colorFalloff);

		if (depth > maxDepth)
		{
			red = devSpheres[i].red * colorFalloff;
			green = devSpheres[i].green * colorFalloff;
			blue = devSpheres[i].blue * colorFalloff;
			maxDepth = depth;
		}
	}
...
int main(void)
{
	...
	cudaMemcpyToSymbol(devSpheres, hostSpheres, sizeof(Sphere) * SPHERES);
	free(hostSpheres);
