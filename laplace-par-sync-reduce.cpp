    ...
    int myRank, int numProcesses, GridFragment *frag, double omega, double epsilon)
{
    ...
    do
    {
        ...
        {
            // We could use MPI_sendrecv instead, but I believe it can be async in the implementation
            if (myRank % 2 == 0) // send then recieve
            {
                if (myRank != numProcesses - 1)
                {
                    MPI_Send(frag->data[!color][last_row - 1],
                             num_color_cols,
                             MPI_DOUBLE,
                             myRank + 1,
                             0,
                             MPI_COMM_WORLD);
                    MPI_Recv(frag->data[!color][last_row],
                             num_color_cols,
                             MPI_DOUBLE,
                             myRank + 1,
                             1,
                ...
                if (myRank > 0)
                {
                    ...
                    MPI_Send(frag->data[!color][1],
                             num_color_cols,
                             MPI_DOUBLE,
                             myRank - 1,
                             2,
                             MPI_COMM_WORLD);
                    MPI_Recv(frag->data[!color][0],
                             num_color_cols,
                             MPI_DOUBLE,
                             myRank - 1,
                             3,
